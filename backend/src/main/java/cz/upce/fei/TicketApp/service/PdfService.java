...
public class PdfService {
    ...
    private static final Color ACCENT_COLOR = new Color(34, 211, 238); // #22d3ee
    ...
    public byte[] generateTicketPdf(Ticket ticket) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            // Vnořená tabulka (Info vlevo, QR vpravo)
            PdfPTable contentTable = new PdfPTable(new float[]{2, 1});
            ...
            try {
                ...
                rightCell.addElement(new Phrase("[QR Error]", fontSmall));
            }
            ...
            document.close(); // Důležité!

            return out.toByteArray();

        } catch (Exception e) {
            e.printStackTrace(); // Uvidíš chybu v konzoli
            throw new RuntimeException("Chyba při generování PDF: " + e.getMessage(), e);
        }
    }

    /**
     * Bezpečné načtení fontu. Pokud selže, vrátí defaultní Helvetica.
     */
    private BaseFont loadFont() {
        try {
            // Cesta: src/main/resources/fonts/Roboto-Regular.ttf
            ClassPathResource fontResource = new ClassPathResource("fonts/Roboto-Regular.ttf");

            if (!fontResource.exists()) {
                System.err.println("!!! POZOR: Font Roboto-Regular.ttf nebyl nalezen ve složce resources/fonts/ !!!");
                return BaseFont.createFont(BaseFont.HELVETICA, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
            }

            // Načteme font jako pole bytů (nejspolehlivější metoda)
            byte[] fontBytes = fontResource.getInputStream().readAllBytes();

            // "Roboto.ttf" je jen virtuální název, důležité jsou ty byty
            return BaseFont.createFont("Roboto.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, true, fontBytes, null);

        } catch (IOException | DocumentException e) {
            System.err.println("Chyba při načítání fontu (používám default): " + e.getMessage());
            try {
                return BaseFont.createFont(BaseFont.HELVETICA, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
            } catch (Exception ex) {
                throw new RuntimeException("Kritická chyba fontů", ex);
            }
        }
    }
}